...
#include <queue>
#include <stack>
#include "./lib55/lp_lib.h"
#include "./lib55/defines.h"
//#include "defines.h"
...
void Graph::_Add_Constraint(int p,char type){
	...
	*/	add_constraintex(lp,cc,row,colno,EQ,1);
	...
	//	char dsaf[20];
	//	for(int i = 0; i < 19; i++)dsaf[i] = rand()%127+1;
	//	dsaf[20] = 0;
		add_constraintex(lp,rcc,row,colno,LE,1);
//		add_SOS(lp,"SOS",1,1,rcc,colno,row);
		for(int i = 0; i < cc; i++){
			row[rcc+i] = -1;
			colno[rcc+i] = F[p][i]+1;
		}
...
//	freopen("logs.txt","w",stdout);
	...
	//  Restore the line before me if errors occured.
	//	add_constraintex(lp,3,row1,colno1,EQ,0);
		row1[0] = -1;
		row1[1] = 1;
		row1[2] = LMT;
		colno1[0] = G.Ec+U;
		colno1[1] = G.Ec+V;
		colno1[2] = i+1;
		add_constraintex(lp,3,row1,colno1,LE,LMT+1);
	//	row1[2] = -LMT;
	//	add_constraintex(lp,3,row1,colno1,GE,-LMT-1);
