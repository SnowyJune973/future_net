...
void Graph::_Add_Constraint(int p,char type){
	...
	*/	add_constraintex(lp,cc,row,colno,EQ,1);
		//add_SOS(lp,"SOS",1,12,cc,colno,NULL);
	}
	if(type == 1){
		memset(row,0,sizeof(row));
		memset(colno,0,sizeof(colno));
		...
		//add_SOS(lp,"SOS",1,12,rcc,colno,NULL);
		memset(row,0,sizeof(row));
		memset(colno,0,sizeof(colno));
		for(int i = 0; i < cc; i++){
			row[i] = 1;
...
int main(int argc,char* argv[]){
    ...
    lp = make_lp(0,G.Ec+G.Nc);
	LMT = min(G.Nc,150);
	memset(colno2,0,sizeof(colno2));
	for(int i = 1; i <= G.Ec; i++)colno2[i-1] = i;
	//add_SOS(lp,"SOS",LMT,12,G.Ec,colno2,NULL);
	if(lp == NULL){
	...
	for(int i = G.Ec+1; i <= G.Ec+G.Nc; i++){
		row[i] = 0;
		set_int(lp,i,TRUE);
		if(i == G.Ec + G.Start)set_bounds(lp,i,1,1);
		else if(G.isprimary[i-G.Ec] || i == G.Ec+G.Terminate)set_bounds(lp,i,1,LMT);
		else set_bounds(lp,i,0,LMT);
	}
	set_obj_fn(lp,row);
//	set_timeout(lp,19500);
	...
	for(int i = 0; i < G.Ec; i++){
		...
		printf("U = %d , V = %d\n",U,V);
		//if(!(G.isprimary[U] || G.isprimary[V]))continue;
		row1[0] = 1;
		row1[1] = -1;
		row1[2] = LMT+1;
		colno1[0] = G.Ec+U;
...
//		swap(colno1[0],colno1[1]);
//		add_constraintex(lp,3,row1,colno1,LE,LMT);
	}
	getchar();
	//set_simplextype(lp,9);
	...
	if(ret == SUBOPTIMAL || ret == OPTIMAL){
		...
		if(ret == TIMEOUT)fprintf(fout0,"NA");
		if(ret == UNBOUNDED)fprintf(fout0,"NA");
	}
	delete_lp(lp);
	//fprintf(fout0,"\n%d",MinRoute[700]);
    fclose(fout0);
